document.addEventListener("DOMContentLoaded", function () {
  ...
  function randomDyGenerate() {
    return randommYDirection[
      Math.floor(Math.random() * randommYDirection.length)
    ];
  }
  ...
  function moveBars(e) {
    ...
    if (isRightBarMove) {
      // while ball will be middle of the board, next player will be able to move the bar.
  ...
  function isBallHitingLeftBar() {
    const leftBar =
      leftBarPosition.x === ballPosition.x &&
  ...
  function isBallHitingRightBar() {
    const rightBar =
      rightBarPositon.x === ballPosition.x + ballDiameter &&
      rightBarPositon.y <= ballPosition.y &&
      ballPosition.y <= rightBarPositon.y + barHeight;
  ...
  function rungameAgain(bar) {
    if (gameRound === 0) {
      ...
      alert(
        ...
        } player win the game by ${Math.abs(
